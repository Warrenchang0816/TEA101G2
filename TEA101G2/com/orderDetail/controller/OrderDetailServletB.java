...
import com.orderDetail.model.OrderDetailServiceB;
...
@WebServlet("/OrderDetailServletB")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class OrderDetailServletB extends HttpServlet {
//	private static final long serialVersionUID = 1L;
    public OrderDetailServletB() {
        super();
    }
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("backend_AddOrderDetail".equals(action)) {
			...
			try {
				...
				OrderDetailServiceB orderDetailServ = new OrderDetailServiceB();
		...
		if ("backend_SelectOrderDetailByMaster".equals(action)) {
			Queue<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/backend/orderMaster/selectOrderMaster.jsp");
					failureView.forward(req, res);
					return;
				}

				OrderDetailServiceB orderDetailServ = new OrderDetailServiceB();
				List<OrderDetailVO> selectOneOrderDetailByMaster = orderDetailServ.selectAllOrderDetailByMaster(orderMasterId);
				
				
				if (selectOneOrderDetailByMaster == null) {
					errorMsgs.add("查無資料");
				}
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/backend/orderMaster/selectOrderMaster.jsp");
				...
				req.setAttribute("selectOneOrderDetailByMaster", selectOneOrderDetailByMaster);

				String url = "/backend/orderDetail/orderDetail.jsp";
				RequestDispatcher sucessVeiw = req.getRequestDispatcher(url);
				sucessVeiw.forward(req, res);
