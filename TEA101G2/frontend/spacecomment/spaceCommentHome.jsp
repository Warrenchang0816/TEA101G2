...
<div id="page">
  ...
  <main>
		...
		<section class="hero_in hotels">
			<div class="wrapper">
				<div class="container">
					<h1 class="fadeInUp"><span></span>場地評價首頁</h1>
				</div>
			</div>
		</section>
		<!--/hero_in-->
...
<ul>
  <li><a href='<%=request.getContextPath()%>/frontend/spacecomment/listAllSpaceComment.jsp'>List</a> all SpaceComments.  <br><br></li>
  
  
  <li>
    <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/spacecomment/spacecomment.do" >
        <b>輸入場地評價ID:</b>
        <input type="text" name="spaceCommentId">
...
<jsp:useBean id="spaceCommentSvc" scope="page" class="com.spaceComment.model.SpaceCommentService" /> 
  ...
  <li>
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/spacecomment/spacecomment.do" >
       <b>選擇場地ID:</b>
       <select size="1" name="spaceCommentId">
         <c:forEach var="spaceCommentVO" items="${spaceCommentSvc.all}" > 
          <option value="${spaceCommentVO.spaceCommentId}">${spaceCommentVO.spaceId}
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_Display">
       <input type="submit" value="送出">
  ...
  <li>
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/spacecomment/spacecomment.do" >
       ...
       <select size="1" name="spaceCommentId">
         <c:forEach var="spaceCommentVO" items="${spaceCommentSvc.all}" > 
          <option value="${spaceCommentVO.spaceCommentId}">${spaceCommentVO.memId}
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_Display">
       <input type="submit" value="送出">
...
<ul>
  <li><a href='<%=request.getContextPath()%>/frontend/spacecomment/addSpaceComment.jsp'>Add</a> a new SpaceComment.</li>
</ul>
</main>
<!--/main-->
</div>
